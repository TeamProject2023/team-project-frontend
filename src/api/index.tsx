...
$api.interceptors.response.use((config) => {
    ...
    if (error.response.status &&
        error.response.status === 401 && error.config && !originalRequest._isRetry) {
        originalRequest._isRetry = true;
        try {
            const response = await axios.get<IRefreshResponse>("/refresh_token", { withCredentials: true, baseURL });
            localStorage.setItem(LocalStorageRefreshToken, response.data.token);
